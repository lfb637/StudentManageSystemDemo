...
public class StudentDaoimp extends BaseDao implements StudentDao {
	...
	//获取每页数据
	public List<Student> getPageStudent(int startIndex, int pageSize) {
		List<Student> list = new ArrayList<Student>();
		String sql = "select `id`,`name`,`gradeId`,`age`,`sex`,`profile` from student limit startIndex,pageSize";
		Object[] params = {};
		ResultSet rs = this.executeQuerySQL(sql, params);
		try {
			while (rs.next()) {
		...
		try {
			while(rs.next()){
				try {
					...
					stu = new Student(id,name,gradeId,sex,age,profile);
					list.add(stu);
